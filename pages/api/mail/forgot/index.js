...
export default nextConnect().post(async (req, res) => {
  try {
    if (!req.body.email) return res.status(400).send("Missing parameters.");

    const personal = await db.Personal.findOne({
      where: { email: req.body.email },
      include: ["personal_secret"],
    });
    const reset_password_token = await require("crypto")
      .randomBytes(32)
      .toString("hex");
    ...
    const reset_password_token_url = `${process.env.EMAIL_TOKEN_URL}/reset-password/${reset_password_token}`;
    let info = await transporter.sendMail({
      ...
      subject: `Admission ${process.env.UNIVERSITY_NAME} | Reset Password`, // Subject line
