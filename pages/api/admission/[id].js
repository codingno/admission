...
export default nc({
  ...
  .post(async (req, res) => {
    ...
    if (
      (personal.dataValues?.personal_detail[0] ||
        personal.personal_detail[0]) &&
      personal.submitted
    ) {
      ...
      Object.keys(prepareData).map((item) => {
        if (item === "personal_id") return;
        if (item === "national_identity_number") {
          if (
            prepareData[item] === "" &&
            prepareData.country_citizen.toLowerCase() === "indonesia"
          )
            throw Error(
              `Please fill form ${item
                .split("_")
                .map((x) => capitalize(x))
                .join(" ")} in Personal Details`
            );
        }
        if (item === "passport_no") {
          if (
            prepareData[item] === "" &&
            prepareData.national_identity_number === ""
          )
            throw Error(
              `Please fill form between ${item
                .split("_")
                .map((x) => capitalize(x))
                .join(" ")} or ${"national_identity_number"
                .split("_")
                .map((x) => capitalize(x))
                .join(" ")} in Personal Details`
            );
          if (
            prepareData[item] !== "" &&
            (prepareData.issue_date === "" || prepareData.expiry_date === "")
          )
            throw Error(
              `Please choose date for ${"issue_date"
                .split("_")
                .map((x) => capitalize(x))
                .join(" ")} and ${"expiry_date"
                .split("_")
                .map((x) => capitalize(x))
                .join(" ")} in Personal Details`
            );
    ...
    if (
      (personal.dataValues?.contact_detail[0] || personal.contact_detail[0]) &&
      personal.submitted
    ) {
    ...
    if (
      (personal.dataValues?.financial_support[0] ||
        personal.financial_support[0]) &&
      personal.submitted
    ) {
    ...
    if (
      (personal.dataValues?.proposed_study[0] || personal.proposed_study[0]) &&
      personal.submitted
    ) {
        ...
        if (!prepareData[item] && prepareData[item] !== "")
          throw Error(
            `Missing Parameter ${item
              .split("_")
              .map((x) => capitalize(x))
              .join(" ")} of Proposed Study Program`
          );
      });
    ...
    if (
      (personal.dataValues?.education_background[0] ||
        personal.education_background[0]) &&
      personal.submitted
    ) {
        ...
        if (!prepareData[item] && prepareData[item] !== "")
          throw Error(
            `Missing Parameter ${item
              .split("_")
              .map((x) => capitalize(x))
              .join(" ")} of Undergraduate Degree in Education Background`
          );
      });

      if (personal.dataValues?.proposed_study[0]) {
        const degree_type = personal.dataValues?.education_background.filter(
          (x) => x.degree_type === "undergraduate"
        );
          ...
          if (!prepareDataMaster[item] && prepareDataMaster[item] !== "")
            throw Error(
              `Missing Parameter ${item
                .split("_")
                .map((x) => capitalize(x))
                .join(" ")} of Master Degree in Education Background`
            );
        });
        if (personal.dataValues?.proposed_study[0]) {
          const degree_type = personal.dataValues?.education_background.filter(
            (x) => x.degree_type === "master"
          );
    ...
    if (
      (personal.dataValues?.language_ability[0] ||
        personal.language_ability[0]) &&
      personal.submitted
    ) {
        ...
        if (!prepareData[item] && prepareData[item] !== "")
          throw Error(
            `Missing Parameter ${item
              .split("_")
              .map((x) => capitalize(x))
              .join(" ")} of Language Ability`
          );
      });

      if (personal.dataValues?.language_ability[0]) {
        const language_type = personal.dataValues?.language_ability.filter(
          (x) => x.language_type === "English"
        );
      ...
      if (req.body.language_type) {
        if (!req.body.proficiency_score)
          throw Error(
            `Missing Parameter ${"proficiency_score"
              .split("_")
              .map((x) => capitalize(x))
              .join(" ")} if insert other language in Language Ability`
          );

        let prepareData = {
          personal_id: personal.id,
          language_type: req.body.language_type,
          proficiency_score: req.body.proficiency_score,
        };
        if (personal.dataValues?.language_ability[0]) {
          const language_type = personal.dataValues?.language_ability.filter(
            (x) => x.language_type === req.body.language_type
          );
          prepareData.id = language_type[0]?.id;
        }
    ...
    if (
      (personal.dataValues?.language_score[0] || personal.language_score[0]) &&
      personal.submitted
    ) {
      ...
      if (personal.dataValues?.language_score[0]) {
        const test_name = personal.dataValues?.language_score.filter(
          (x) => x.test_name === req.body.test_name
        );
    ...
    if (
      (personal.dataValues?.job_status[0] || personal.job_status[0]) &&
      personal.submitted
    ) {
      ...
      if (req.body.job_status_type === "Currently employed") {
        ...
        if (personal.dataValues?.job_status[0]) {
          const job_data = personal.dataValues?.job_status.filter(
            (x) => x.employee_type === 1
          );
        ...
        if (personal.job_status[0]) {
          Object.keys(prepareData).map((job_status_obj) => {
            personal.job_status[0][job_status_obj] =
              prepareData[job_status_obj];
          });
          personal.job_status[0].save();
        } else allData.job_status = prepareData;
      } else {
        let prepareData = {
          personal_id: personal.id,
    ...
    if (
      (personal.dataValues?.referee_detail[0] || personal.referee_detail[0]) &&
      personal.submitted
    ) {
      ...
      if (req.body.referee2_title) {
        let prepareData2 = {
          ...
          relationship_to_applicant:
            req.body.referee2_relationship_to_applicant,
    ...
    const personal_detail = await db.PersonalDetail.bulkCreate(
      [allData.personal_detail],
      { updateOnDuplicate: filterAttributes(db.PersonalDetail) }
    );
    const contact_detail = await db.ContactDetail.bulkCreate(
      [allData.contact_detail],
      { updateOnDuplicate: filterAttributes(db.ContactDetail) }
    );
    const financial_support = await db.FinancialSupport.bulkCreate(
      [allData.financial_support],
      { updateOnDuplicate: filterAttributes(db.FinancialSupport) }
    );
    const proposed_study = await db.ProposedStudy.bulkCreate(
      [allData.proposed_study],
      { updateOnDuplicate: filterAttributes(db.ProposedStudy) }
    );
    const education_background = await db.EducationBackground.bulkCreate(
      allData.education_background,
      { updateOnDuplicate: filterAttributes(db.EducationBackground) }
    );
    const language_ability = await db.LanguageAbility.bulkCreate(
      allData.language_ability,
      { updateOnDuplicate: filterAttributes(db.LanguageAbility) }
    );
    const language_score = await db.LanguageScore.bulkCreate(
      [allData.language_score],
      { updateOnDuplicate: filterAttributes(db.LanguageScore) }
    );
    const job_status = await db.JobStatus.bulkCreate([allData.job_status], {
      updateOnDuplicate: filterAttributes(db.JobStatus),
    });
    const referee_details = await db.RefereeDetail.bulkCreate(
      allData.referee_detail,
      { updateOnDuplicate: filterAttributes(db.RefereeDetail) }
    );
  ...
  .patch(async (req, res) => {
    ...
    if (!personal) throw Error("User not found.");
    ...
    for await (const referee of personal.referee_detail) {
      ...
      const email_token_url = `${process.env.EMAIL_TOKEN_URL}/referee/${email_token}`;
      ...
      let info = await transporter.sendMail({
        ...
        subject: `Admission ${process.env.UNIVERSITY_NAME} Referee Recommendation`, // Subject line
        // text: "Hello world?", // plain text body
        // html: "<b>Hello world?</b>", // html body
        html: `
					<p>Dear ${referee.title} ${referee.family_name} ${referee.given_name}.</p>
					<p>We have recently received an application to Indonesian International Islamic University from ${personal.name} who requested that you provide a reference (letter of recommendation) in support of their application to ${process.env.UNIVERSITY_NAME}.</p>
					<br />
