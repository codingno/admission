...
export default function FormVerification() {
		...
		const [prepareStudentSelected, setPrepareStudentSelected] = useState({id : null, name : "" })
		...
		useEffect(() => {
			...
			async function getStudentSelected() {
				try {
					...
					setPrepareStudentSelected(data[0])
				} catch (error) {
					handleError(error)	
				}	
			}
		...
		useEffect(() => {
			if(id && studentOptions.length > 0)
					setStudentSelected(prepareStudentSelected)
		}, [studentOptions, id])
                                      ...
                                      <RadioGroup 
																			// {...props}
																				...
																				onChange={(event, data) => {
																					...
																					// temps[index].is_relevance = String(data) == 'true'
