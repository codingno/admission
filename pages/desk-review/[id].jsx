...
export default function FormAdmission() {
		...
		useEffect(() => {
			...
			async function getDataSaved() {
				try {
					const { data } = await axios.get('/api/desk-score?personal_id=' + id)
					if(data) {
						setDataSaved(data)
						data.map(eachData => {
					...
					const note = await axios.get(`/api/desk-notes?personal_id=${id}`)
					if(note.data[0]) {
		...
		useEffect(() => {
			...
			async function getAspectAndScoreList() {
				try {
					const { data : dataAspect } = await axios.get('/api/aspect?type=1')
		...
		async function submitForm(dataForm) {
			if(confirm("Save data?"))
				try {
					...
					await axios.post('/api/desk-score', prepareData)
					await axios.post('/api/desk-notes', [notes])
					alert("Data Saved.")
					setReload(true)
					// setLoading(false)
                  ...
                  >
                    ...
                    <Grid item xs={10} lg={5.5}>
                      ...
                      >
                        ...
                        <TextField value={(dataSaved ? dataSaved[0]?.admin.name : null) || session?.user.name} disabled={true}
                          sx={{
                            input: {
                              "&.Mui-disabled": {
                                color: "#000",
                          ...
                          <TableRow
                            ...
                            <TableCell align="center">
                              {watchAllFields[item] ?? ""}
										...
										>
											<Typography variant="h6" color="initial" m={1}>Desk Review Notes : </Typography>
											<TextField 
												value={notes?.notes}
												onChange={e => {
													// clearTimeout(notesChange)
													// notesChange = setTimeout(
